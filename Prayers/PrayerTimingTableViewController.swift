...
class PrayerTimingTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
		...
		if let data = NSData.init(contentsOf: url as URL) {
			do {
				let json = try JSONSerialization.jsonObject(with: data as Data, options: .allowFragments) as? [Any]
				
				if let item = json?[2] as? [[String: Any]] {
					for object in item {
						if let timesArray = object["times"] as? [String] {
							for time in timesArray {
								...
								let date = Date()
								let givenDate = dateFormatter.date(from: time)
								let timeInterval = date.timeIntervalSince(givenDate!)
								let newDate = date.addingTimeInterval(timeInterval)
								
								print(dateFormatter.string(from: newDate))
								
								if let date = object["date"] as? Int {
									print(date)
								}
								
								if let month = object["month"] as? Int {
									print(month)
								}
							}
						}
